...
} from './oandaAutoTradeManager.js';

test('Auto AI may enter through 1:59 PM ET but not at 2:00 PM ET', () => {
  assert.equal(inAutoAiEntryWindow(new Date('2026-07-14T17:59:00Z')), true); // 13:59 ET
  assert.equal(inAutoAiEntryWindow(new Date('2026-07-14T18:00:00Z')), false); // 14:00 ET
});

test('active-trade management remains available through the 5 PM ET sweep', () => {
  assert.equal(inAutoAiManagementWindow(new Date('2026-07-14T20:59:00Z')), true); // 16:59 ET
  assert.equal(inAutoAiManagementWindow(new Date('2026-07-14T21:00:00Z')), true); // 17:00 ET
  assert.equal(inAutoAiManagementWindow(new Date('2026-07-14T21:05:00Z')), false); // 17:05 ET
});
...
test('live V3 reversal adjustment closes even when legacy invalidation is absent', () => {
  const decision = decideAutoAiClose({
    invalidationDetected: false,
    liveTpConfidence: {
