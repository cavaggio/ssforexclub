...
export async function executeTrade(signal, options = {}) {
  ...
  // forex SL/TP to the ACTUAL fill so the forex SL is exactly 10.0 pips when accepted and the TP is exactly 15.0 pips.
  if (!metals && tradeId) {
    const fixedSlDistance = FIXED_FOREX_STOP_LOSS_PIPS * pipSize;
    const fixedTpDistance = FIXED_FOREX_TAKE_PROFIT_PIPS * pipSize;
    ...
    try {
      ...
      executionLog.push(logEntry('FIXED_10P_FILL_REANCHOR_FAILED', { error: err.message }));
    }
  }
