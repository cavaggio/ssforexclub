...
function windowRead(candles) {
  if (!candles.length) return { bias: 'neutral', structuralBull: false, structuralBear: false, net: 0, span: 0, normalizedNet: 0, directionalScore: 0 };
  ...
  const directionalScore = normalizedNet + (structuralBull ? 0.35 : 0) - (structuralBear ? 0.35 : 0);
  const bias = structuralBull && net > 0
    ...
    : structuralBear && net < 0
      ...
      : Math.abs(normalizedNet) >= 0.22
        ...
        : 'neutral';
  return { bias, structuralBull, structuralBear, net, span, normalizedNet, directionalScore };
}

export function computeDailyStructure({ dailyCandles = [], currentPrice = null }) {
  const closed = (Array.isArray(dailyCandles) ? dailyCandles : []).filter((candle) => candle && candle.complete !== false);
  ...
  if (closed.length < DAILY_ACTIVE_MIN) {
    return {
      ...
      recencyScore: 0, activeStrength: 0, contextStrength: 0, invalidatedByRecency: false,
  ...
  const weights = [0.7, 0.85, 1.0].slice(-recent.length); // oldest -> newest
  let weighted = 0;
  let weightTotal = 0;
  ...
  const recencyBias = Math.abs(recencyScore) >= 0.18 ? (recencyScore > 0 ? 'bullish' : 'bearish') : 'neutral';
  ...
  return {
    dailyBias, qualified: true, reason: reasons.join(' '),
    activeBias: activeRead.bias, contextBias: contextRead.bias, recencyBias,
    activeWindow: { candles: active.length, minimum: DAILY_ACTIVE_MIN, maximum: DAILY_ACTIVE_MAX, structuralBull: activeRead.structuralBull, structuralBear: activeRead.structuralBear, net: activeRead.net, normalizedNet: +activeRead.normalizedNet.toFixed(4) },
    contextWindow: { candles: context.length, maximum: DAILY_CONTEXT, structuralBull: contextRead.structuralBull, structuralBear: contextRead.structuralBear, high: Math.max(...context.map((c) => Number(c.high))), low: Math.min(...context.map((c) => Number(c.low))), net: contextRead.net },
    recencyWindow: { candles: recent.length, maximum: DAILY_RECENCY, weightedScore: +recencyScore.toFixed(4) },
    ...
    invalidatedByRecency, currentPrice: current, currentState,
  };
}

export const ICT_DAILY_WINDOWS = Object.freeze({ ACTIVE_MIN: DAILY_ACTIVE_MIN, ACTIVE_MAX: DAILY_ACTIVE_MAX, CONTEXT: DAILY_CONTEXT, RECENCY: DAILY_RECENCY });
