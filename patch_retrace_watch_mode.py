...
watch_file = SERVER / "retraceWatchMode.js"

watch_file.write_text(r"""
// Retrace Watch Mode
...
# Patch likely scheduler/router files to prioritize watched pairs in scan order.
targets = [
...
for p in targets:
    ...
    if "retraceWatchMode.js" not in s:
        s = "import { getRetraceWatchPairs, evaluateRetraceCandidate } from './retraceWatchMode.js';\n" + s

    # Add helper once
    if "function prioritizeRetraceWatchPairs" not in s:
        s += r"""
...
"""
    ...
    # Common pair list names
    s = re.sub(r"\bconst\s+pairs\s*=\s*([^;\n]+);", r"const pairs = prioritizeRetraceWatchPairs(\1);", s, count=1)
    s = re.sub(r"\blet\s+pairs\s*=\s*([^;\n]+);", r"let pairs = prioritizeRetraceWatchPairs(\1);", s, count=1)
    s = re.sub(r"\bconst\s+scanPairs\s*=\s*([^;\n]+);", r"const scanPairs = prioritizeRetraceWatchPairs(\1);", s, count=1)
    s = re.sub(r"\blet\s+scanPairs\s*=\s*([^;\n]+);", r"let scanPairs = prioritizeRetraceWatchPairs(\1);", s, count=1)

    # Add passive evaluator after obvious signal/analysis objects.
    if "evaluateRetraceCandidate(signal" not in s:
        s = s.replace(
            "return signal;",
            "evaluateRetraceCandidate(signal, 'auto');\n    return signal;",
            1
        )
        s = s.replace(
            "return analysis;",
            "evaluateRetraceCandidate(analysis, 'auto');\n    return analysis;",
            1
        )
...
print("Retrace watch patch complete.")
