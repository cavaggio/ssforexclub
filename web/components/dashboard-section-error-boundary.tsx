...
export class DashboardSectionErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    return (
      <section role="alert" style={{ background: 'var(--panel)', border: '1px solid var(--warn)', borderRadius: 10, padding: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
          <button type="button" onClick={this.retry} style={{ border: '1px solid var(--accent)', background: 'var(--accent)', color: '#06101a', borderRadius: 6, padding: '7px 12px', fontWeight: 800, cursor: 'pointer' }}>
            Retry section
          </button>
          <button type="button" onClick={() => window.location.reload()} style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', borderRadius: 6, padding: '7px 12px', fontWeight: 700, cursor: 'pointer' }}>
            Reload dashboard
          </button>
        </div>
      </section>
