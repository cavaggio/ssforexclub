...
test('exactly +10 pips moves the stop to breakeven without taking profit', () => {
  const decision = evaluateActiveExit({ ...base, currentPrice: 1.101, unrealizedPips: 10 });
...
test('exactly +15 pips closes 80% and keeps the remaining 20% at breakeven', () => {
  const decision = evaluateActiveExit({ ...base, currentPrice: 1.1015, unrealizedPips: 15 });
...
test('between +15 and +18 pips the remaining 20% is held at breakeven', () => {
  const decision = evaluateActiveExit({ ...base, currentPrice: 1.1017, unrealizedPips: 17 }, {
    priorPartialCount: 1,
    breakEvenSet: true,
  });
  assert.equal(decision.action, 'HOLD_TO_TP');
...
test('exactly +18 pips closes the remaining 20%', () => {
  const decision = evaluateActiveExit({ ...base, currentPrice: 1.1018, unrealizedPips: 18 }, {
    priorPartialCount: 1,
    breakEvenSet: true,
  });
  assert.equal(decision.action, 'PARTIAL_CLOSE');
