#!/usr/bin/env python3
"""Enforce PPR's 80% execution floor, live readiness diagnostics, and scan counts.
...
the former 85% PPR threshold or hide the environment flags that block orders.
"""
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if new in text:
        return text
    if old not in text:
        raise RuntimeError(f"PPR live-execution alignment marker missing: {label}")
...
for marker in [
    ...
    if marker not in engine:
        raise RuntimeError(f"PPR engine live alignment incomplete: missing {marker}")
...
print("PPR live execution aligned: 80% floor, readiness diagnostics, and exact scan accounting")
