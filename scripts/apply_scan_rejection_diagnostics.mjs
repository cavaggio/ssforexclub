...
export function patchSchedulerWindowDiagnostic(source) {
  ...
  const legacyAccurate = '[AUTO_AI] endOfDayReview=17:30_ET scans=02:00–10:00_ET entries=02:30–10:00_ET weekdays_only ';
  if (source.includes(legacyAccurate)) return source.replace(legacyAccurate, current);

  const oldUnified = '[AUTO_AI] scans=02:00–10:00_ET entries=V3/PPR/ICT_02:15 weekdays_only ';
  if (source.includes(oldUnified)) return source.replace(oldUnified, current);

  const stale = '[AUTO_AI] scans=02:00–10:00_ET entries=V3_02:15/PPR_03:00/ICT_05:00 weekdays_only ';
  return replaceRequired(source, stale, current, 'scheduler execution-window diagnostic');
}
